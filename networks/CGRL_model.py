...
class Generator(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.fc4 = nn.Linear(2*layer_sizes[0], layer_sizes[0])
        self.lrelu = nn.LeakyReLU(0.2, True)
    ...
    def forward(self, z, a1=None, c=None, feedback_layers=None):
        if feedback_layers is None:
            ...
            # feedback_out = x1*feedback_layers
    ...
    #         z = torch.cat((z, c), dim=-1)
    #         x1 = self.lrelu(self.fc1(z))
    #         feedback_out = torch.cat((x1, a1*feedback_layers),dim=-1)
    #         x4 = self.lrelu(self.fc4(feedback_out))
    #         x = self.sigmoid(self.fc3(x4))
    #         return x
...
class Feedback(nn.Module):
    def __init__(self,opt):
        ...
        self.fc1 = nn.Linear(opt.neh, opt.ngh)
...
class AttClass(nn.Module):
    def __init__(self, opt):
        super(AttClass, self).__init__()
        self.embedSz = 0
        self.fc1 = nn.Linear(opt.resSize + self.embedSz, opt.ngh)
        self.fc3 = nn.Linear(opt.ngh, opt.nclass_seen)
        self.lrelu = nn.LeakyReLU(0.2, True)
        self.hidden = None
        self.sigmoid = None
        self.apply(weights_init)
...
class Embedding_Net(nn.Module):
    ...
    def forward(self, features):
        self.hidden = self.relu(self.fc1(features))

        out_z = F.normalize(self.fc2(self.hidden), dim=1)
        return out_z
    
    def getLayersOutEmbed(self):
