...
class   SupConLoss_clear(nn.Module):
    ...
    def forward(self, features, labels):
        ...
        logits_mask = torch.ones_like(mask).scatter_(1,torch.arange(batch_size).view(-1, 1).cuda().long(),0.0)

        mask = mask * logits_mask
        single_samples = (mask.sum(1) == 0).float()

        # compute log_prob
        exp_logits = torch.exp(logits) * Variable(logits_mask)
        log_prob = logits - torch.log(exp_logits.sum(1, keepdim=True))

        # compute mean of log-likelihood over positive
        # invoid to devide the zero
        mean_log_prob_pos = (Variable(mask) * log_prob).sum(1) / Variable((mask.sum(1)+single_samples))

        # loss
        # filter those single sample
        loss = - mean_log_prob_pos*Variable((1-single_samples))
